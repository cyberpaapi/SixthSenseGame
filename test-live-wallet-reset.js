  ...
  try {
    ...
    await context.addInitScript(() => {
      if (!localStorage.getItem("qa-wallet-seeded")) {
        ...
        localStorage.setItem("sixth-sense.stats.v1", JSON.stringify({ economyVersion: 8, coins: 9876, played: 12, inventory: { peek: 3 }, adventure: { seed: 12345, level: 8 } }));
    ...
    let policy = { resetVersion: 8, startingCoins: 500 };
    let offline = false;
    ...
    policy = { resetVersion: 9, startingCoins: 500 };
    ...
    assert.equal(await first.evaluate(() => JSON.parse(localStorage.getItem("sixth-sense.stats.v1")).economyVersion), 9, "bundled version must not downgrade a newer reset");
    offline = true;
    ...
    await first.waitForFunction(() => JSON.parse(localStorage.getItem("sixth-sense.stats.v1")).economyVersion === 10, null, { timeout: 35000 });
