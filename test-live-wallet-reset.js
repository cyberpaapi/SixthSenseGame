  ...
  try {
    ...
    await context.addInitScript(() => {
      if (!localStorage.getItem("qa-wallet-seeded")) {
        ...
        localStorage.setItem("sixth-sense.stats.v1", JSON.stringify({ economyVersion: 10, coins: 9876, played: 12, inventory: { peek: 3 }, adventure: { seed: 12345, level: 8 } }));
    ...
    let policy = { resetVersion: 10, startingCoins: 500 };
    let offline = false;
    ...
    assert.equal(await first.evaluate(() => JSON.parse(localStorage.getItem("sixth-sense.stats.v1")).economyVersion), 11, "bundled version must not downgrade a newer reset");
    offline = true;
    ...
    await first.evaluate(() => window.SixthSenseEconomy.credit(40));
    policy = { resetVersion: 12, startingCoins: 500 };
    await first.waitForFunction(() => JSON.parse(localStorage.getItem("sixth-sense.stats.v1")).economyVersion === 12, null, { timeout: 35000 });
