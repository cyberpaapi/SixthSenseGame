  ...
  try {
    ...
    await context.addInitScript(() => {
      if (!localStorage.getItem("qa-wallet-seeded")) {
        ...
        localStorage.setItem("sixth-sense.stats.v1", JSON.stringify({ economyVersion: 7, coins: 9876, played: 12, inventory: { peek: 3 }, adventure: { seed: 12345, level: 8 } }));
    ...
    let policy = { resetVersion: 7, startingCoins: 250 };
    let offline = false;
    ...
    policy = { resetVersion: 8, startingCoins: 250 };
    await first.evaluate(() => window.dispatchEvent(new Event("focus")));
    for (const page of [first, second]) await page.waitForFunction(() => document.querySelector("#coin-count").textContent === "250");
    ...
    await second.waitForFunction(() => document.querySelector("#coin-count").textContent === "290");
    await second.evaluate(() => window.dispatchEvent(new Event("focus")));
    await second.waitForTimeout(200);
    assert.equal(await second.locator("#coin-count").textContent(), "290", "same reset must not erase later earnings");
    await first.reload();
    await first.waitForFunction(() => document.querySelector("#coin-count").textContent === "290");
    assert.equal(await first.evaluate(() => JSON.parse(localStorage.getItem("sixth-sense.stats.v1")).economyVersion), 8, "bundled version must not downgrade a newer reset");
    offline = true;
    await first.evaluate(() => window.dispatchEvent(new Event("online")));
    await first.waitForTimeout(200);
    assert.equal(await first.locator("#coin-count").textContent(), "290");
    offline = false;
    ...
    await first.waitForFunction(() => document.querySelector("#coin-count").textContent === "250");
    await second.waitForFunction(() => document.querySelector("#coin-count").textContent === "250");
    await first.evaluate(() => window.SixthSenseEconomy.credit(40));
    policy = { resetVersion: 9, startingCoins: 250 };
    await first.waitForFunction(() => JSON.parse(localStorage.getItem("sixth-sense.stats.v1")).economyVersion === 9, null, { timeout: 35000 });
    assert.equal(await first.locator("#coin-count").textContent(), "250", "periodic check must reset an already-open updated page");
    console.log("Live wallet checks passed: foreground/30-second policy resets, cross-tab balance sync, stale legacy-write repair, exact 250, preserved puzzle/progress, offline recovery and no repeated reset after earning/reload.");
