...
Unassisted snapshots send only the active answer's length and generic kind. Sense returns its clue after unlocking; Peek returns one authorized position. The full bank and route are not sent to players. All `/data/*` paths are blocked on Vercel and omitted from Pages/mobile bundles. Public repository readers can inspect vocabulary, but the random active route remains server-side. No posters, studio logos, likenesses or soundtrack media were copied; the launcher is an original generic clapperboard SVG.
