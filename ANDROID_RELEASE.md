...
Deploy the matching multiplayer backend/client before expecting new online rooms to use six tries or offer rewarded Last Chance. New server snapshots advertise the limit and reward capability; clients keep the legacy seven-try server behavior until then. Existing rooms keep their original seven-try contract via the migrated max_guesses column; newly created rooms use six. Extra tries remain server-authorized, with native earned callbacks and room/round/batch-scoped retries. There is still no AdMob SSV or tamper-proof wallet.
