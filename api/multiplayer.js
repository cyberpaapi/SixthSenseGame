...
async function ensureSchema(sql) {
  if (!schemaPromise) schemaPromise = (async () => {
    await sql`CREATE TABLE IF NOT EXISTS sixth_sense_rooms (
      ...
      mode text NOT NULL CHECK (mode IN ('race','vs')),
...
async function snapshot(sql, room, me) {
  ...
  return {
    ...
    me: {
      ...
      currentWordIndex: room.mode === "vs" ? room.current_round : own.current_word_index,
...
async function createRoom(sql, body) {
  const mode = body.mode === "vs" ? "vs" : "race";
...
async function submitVsGuess(sql, { code, guess, actionId, me, room }) {
  ...
  const exhausted = !won && attempts.length >= Core.MAX_GUESSES;
...
module.exports._test = { cleanPlayer, roomCode, token, tokenHash, normalizeGameLength, resolveVsRound, chooseAnswers, ACCENTS, AVATARS };
