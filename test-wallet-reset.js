...
(async () => {
  ...
  try {
    for (const version of [undefined, 1, 2, 3, 4, 5, 6, 7]) {
      ...
      assert.equal(state.stats.coins, version === 7 ? 9876 : 250);
      assert.equal(state.stats.economyVersion, 7);
      ...
      assert.equal(earned, (version === 7 ? 9876 : 250) + 140);
      await page.reload();
    ...
    console.log("Wallet reset passed: legacy versions including v6, zero/high balances, v7 preservation, progress/inventory/cosmetics/receipt preservation, actual earnings and reload persistence, fresh 250 baseline.");
