...
Last verified: 2026-09-21 (Selected happy-fox branding: exact attached source preserved, opaque Play 512×512 and 1024×500 exports checked, transparent adaptive/legacy-round icon previews inspected, web favicon/install-icon references checked, Android sync/debug assembly/lint and whitespace passed. Earlier monetization suites and deployments remain verified below. Current source/code 4 needs a signed upload and live purchase/ad acceptance; Play Alpha still has code 3. Merchant profile and paid catalog setup remain pending. Fox logo selection is complete; updated store asset upload and closed-test submission remain pending.)
...
Sixth Sense is an original, mobile-first word deduction game with solo and private-room multiplayer. Standard-mode answers have six letters; Bollywood Race and Bollywood VS use 5–7-letter complete movie titles and fictional character names/nicknames. New puzzles have six standard guesses, followed by one optional Last Chance. Feedback is presented through both color and symbols:
...
- Word length: 6 letters in standard modes; 5–7 in Bollywood Race and VS.
- Standard guesses: 6; maximum 7 after one optional Last Chance (legacy saves/rooms retain their recorded limit).
- Accepted input: physical keyboard, onscreen keyboard, touch, optional physical Enter, and Backspace/Delete. Entering the final required letter submits the row immediately (six in standard modes, the displayed 5–7 length in Bollywood Race and VS); there is no onscreen Enter key.
- Standard-mode guesses must contain exactly six letters and exist in the accepted-guess dictionary. Bollywood Race and VS accept alphabetic guesses of the displayed answer length, including names/titles outside the English dictionary; its hidden answer pool is server-only.
...
The home screen exposes seven modes:
...
| Bollywood Race | A 2–8-player Race variant with 3/5/10-word routes and 5–7-letter answers. Its replacement 125-answer pool has 75 complete movie titles and 50 fictional characters, with 98 post-2000 references and 27 classics. The board shows Film or Character and length; Sense unlocks one short clue. Omit spaces/punctuation from complete titles; character answers use familiar names/nicknames. No actor-name answers or difficulty selector. Six guesses plus Last Chance, late joins, saved-seat resume, screen-away alerts and solo-only Skip rules remain shared with Race. |
...
| Bollywood VS | A two-player Bollywood duel using the same replacement 125-answer movie/character bank as Bollywood Race. Choose 3, 5, 9 or Endless rounds. Both players share each answer; first solve wins one point and the existing 60% VS coin reward. Six guesses plus optional Last Chance, redacted opponent patterns, resume, hints and screen-away signals remain. Endless selection stays in the 125-word pool after solves and forfeits, avoiding repeats until exhausted. No difficulty selector or multiplayer Skip. |
...
Practice, Time Tackle, Insight and Streak no longer have home launchers or help promotion. Their internal engines and saved-state keys remain for compatibility, with test-only DOM fixtures exercising existing solo reward/save behavior. This change does not reset coins, progress, inventories or saved rooms.
...
- Solo win cards show mastery progress, actual personal-best timing/attempt improvements, and trio progress/bonus when relevant. Coin totals include only coins actually credited under the 99,999 cap. The retired internal Practice, Time Tackle, Insight, and Streak flows retain optional **Next word** beside the existing green OK. Time Tackle's action explicitly says it starts a fresh ten-minute word. Daily OK still returns home; Adventure OK still returns to its map. No automatic next-word trigger, coercive timer, random reward schedule, energy gate, or new ads were added.
...
The Android project uses Capacitor 8.5.1, Java 21, target/compile API 36 and package `com.sensei.sixthsense`. Source is version **1.0.3 / code 4**; Play's saved Alpha draft still contains older signed 1.0.2/code 3. Developer account is AlphaCodeAI, existing app publisher copy is Sensei, support is `aryanchandwani@gmail.com`, intended audience 13+. `ANDROID_RELEASE.md` and `MONETIZATION.md` distinguish implementation from pending external activation.

`SenseiMonetizationPlugin.java` uses Google Mobile Ads 25.4.0, UMP 4.0.0 and Play Billing 8.3.0. Banners retain reserved native space on home and below the solo/online lifelines with an 8 dp separator; they hide during maps/dialogs/full-screen ads. Boards resize within the remaining WebView and keys/lifelines retain 44 px targets. Configuration changes recreate adaptive banners and ignore obsolete load callbacks.

Automatic interstitial opportunities occur after a whole multiplayer match finishes and after every three new Adventure level advancements (solve or paid Skip), before the result card. Replays, losses, Daily, and individual multiplayer words/VS rounds do not count. `AdBreakPolicy` persists deduplicated event IDs and the Adventure modulo-three count. Only a preloaded ad younger than 55 minutes can show, while foregrounded, consented, eligible and without another full-screen flow. Missing/failed ads skip the opportunity and allow the result immediately; there is no delayed queue. Audio pauses during ads and the game's ad-active signal suppresses multiplayer away alerts. Debug/internal use Google's sample interstitial ID; production validation rejects a missing/sample real interstitial ID.

Optional reward-card ads still grant an extra 2× original solve coins only after the native earned callback, for 3× total solve coins within the wallet cap. Points, mastery, trio and streak bonuses never multiply. Losses/skips/replays/already-claimed rewards are excluded. Native pending reward receipts and the atomic `stats.rewardedClaims` ledger recover interrupted delivery. Last Chance remains a separate earned-ad option. There is no AdMob server-side reward verification.

The approved Android catalog is in `store-catalog.js`: Remove Ads ₹799 (`remove_banner_ads`), 500 coins ₹99 (`coins_500`), 1,500 coins ₹249 (`coins_1500`), 3,500 coins ₹499 (`coins_3500`), Lifeline Kit ₹149 (`lifeline_kit`: 5 Sense + 3 Peek + 3 Clear), Complete Pack ₹999 (`complete_pack`: permanent Remove Ads + 1,500 coins + 10 Sense + 5 Peek + 5 Clear, supplies once). Prices are India setup targets only; native Play product details supply every displayed/charged regional price. There is no subscription or automatic credit/discount when an existing Remove Ads owner buys Complete Pack. Both permanent products remove banners **and automatic interstitials**, keeping optional rewarded ads available.

The native wallet opens the illustrated Settings shop; six generated transparent WebP assets and exact prompts are in `assets/shop/`. Unavailable or already-owned products, pending deliveries and coin packs without full wallet capacity disable checkout. Web has neither paid checkout nor AdMob. All six products verify via `/api/play-store`; `/api/play-purchase` remains compatible with older builds. A random native installation ID is passed to Play's obfuscated account field and checked by the server before supplies are delivered. Neon stores token hashes, product/installation IDs and delivery timestamps, not raw purchase tokens. Native tokens remain private; JS receives sanitized receipt IDs/quantities. Wallet, inventory and `stats.purchaseClaims` save atomically before the backend consumes repeatable purchases. Permanent Complete Pack is acknowledged but never consumed, and its supplies cannot be repeatedly restored. Duplicate/retried receipts do not pay again. A full wallet defers the entire purchased pack, rather than truncating paid coins. Wallets containing paid receipts are excluded from test reset policies.

Permanent ownership is server-verified and natively cached with seven days' offline grace; valid inactive/refund checks revoke earlier. No localStorage flag grants paid ad removal. Supplies remain installation-local and do not restore after reinstall/data clearing or username recovery; the shop and privacy policy disclose this. Complete Pack ad removal restores with the same Play account on another device, while its supplies remain one-time on the original installation. There is no cloud wallet, RTDN, or automatic clawback of refunded consumed supplies. The virtual wallet is not tamper-proof.

Checkout requires `/api/play-store` readiness (`available` and catalog version 1). This verifies environment presence, not Google permissions; real Play license tests are required. New endpoint rate limit is 40 requests/minute per keyed connection bucket, legacy endpoint remains 15; old buckets are cleaned after one day on subsequent requests. Android backup/transfer excludes local progress/receipts. WebView inspection is debug-only.

External setup remains incomplete: Play Console explicitly required creation of a merchant payments profile on September 21, and no paid catalog products/prices were created. The owner must complete that financial setup. The browser is signed into the Play account, whereas the existing AdMob account is separate; no duplicate AdMob account was created. Existing public app/banner/reward IDs remain in ignored `android/release.properties`, but an interstitial unit ID, service-account configuration, license-test purchases, AdMob verification/live serving and physical-device acceptance are still pending. Internal/debug builds use sample ads; production remains gated by real IDs, HTTPS verification, signing and `releaseChecklistApproved=true`. Do not promote a sample-ad test bundle to public production.
...
Starting wallet: 500 coins. The wallet hard-caps at 99,999 across saved-state loading, solo rewards, Daily streak rewards, multiplayer credits, and refunds; a reward that crosses the ceiling grants only the remaining room. Economy version 10 resets existing pre-version-10 saved wallets without paid-purchase receipts to exactly 500 once on their next load, without changing progress, inventory, cosmetics, or statistics. Solving rewards fewer coins as more attempts are used:
...
- Solo and every multiplayer variant use the same purple, white-text message bubble in a reserved 44px slot above the board. Invalid guesses and Peek/Clear feedback use matching wording. Temporary feedback lasts 2.6 seconds and survives room polls; multiplayer then restores the latest round/waiting status. Connection recovery restores the status even when the room snapshot itself is unchanged. Long messages wrap inside the slot and can scroll instead of widening the page or covering controls. Screen/room exits clear stale messages. Browser solo play also uses the existing fitted two-column native layout on short landscape screens, correcting the overflow found during this check. Sense still uses its shared centered clue dialog; presence alerts, round transitions and outcome cards retain their distinct purposes.
...
- App identity: the owner selected the happy furry orange fox above two rows of SIXTH / SENSE blocks on September 21. The exact attachment is preserved in `store-assets/fox-selected/approved-source.png`; it supplies the opaque 512px Play icon and 192/512px browser/install icons. A generated transparent foreground restores the cropped ear tip and fits Android adaptive/round masks. Android launcher densities and splash reference use the selected fox. The matching generated 1024×500 feature graphic contains no ads or aperture mark. `scripts/export-fox-icons.py` reproducibly exports image sizes/layers; prompts and source provenance are in the selected-art README. The game's compact horizontal clay wordmark and player avatar remain in their existing layout. Local exports are ready; replacement in the Play Console draft has not been confirmed.
- Visual direction: vibrant claymorphism with purple, pink, cyan, yellow, and orange depth lighting.
- Home: generated observatory hero art, a seven-day streak progress rail with a 300-coin reward icon, a Daily call-to-action, a major Adventure launcher, Race/VS/Co-op launchers in Play Together (three columns on desktop; Co-op spans the phone grid), then a two-card Bollywood spotlight shelf replacing the former four solo launchers. Bollywood Race and Bollywood VS use new original generated animal/cinema scenes with visible mode and player-count labels. Race, VS, and Co-op use original independent clay scenes. On phones, the Daily and Adventure feature cards each occupy approximately half of the small viewport.
...
| `test-messages-browser.js` | Explicit solo/all-multiplayer message tests: invalid input, lifeline copy, polling/timer behavior, reconnect recovery, exit cleanup and shared light/dark phone/landscape geometry. `app.js` exposes `SixthSenseMessages` to share presentation and timers with multiplayer. |
...
| `answer-bank.js` | 10,106 answer objects with a six-letter `word`, `clue`, and `tier`. Loaded before `game-core.js` in the browser and required by the server. |
...
| `test-bollywood-pricing.js` | Browser fixtures check standard and Bollywood Race/VS displayed and accessible prices, actual wallet deductions, free Sense reopening, retained tokens on failed requests, retry and insufficient funds. |
| `test-bollywood.js` / `test-bollywood-vs.js` / `test-bollywood-browser.js` / `test-production-bollywood.js` | Data/server invariants, VS theme/capacity and both Endless paths, 5–7-column phone input/layout, and real two-browser production rooms. Set BOLLYWOOD_VARIANT=vs for VS browser tests; default Race covers late join. |
| `test-legacy-solo-helper.js` | Test-only DOM launcher substitution to exercise retained solo engines after their production home launchers were removed. No runtime test hook. |
| `assets/bollywood-race-v1.webp` / `assets/bollywood-vs-v1.webp` / `assets/bollywood-mode-art.md` | Built-in image_gen illustrations, optimized to 1024×1024 WebP (234,836 and 178,130 bytes), and exact generation prompts/provenance. Race uses animal runners and a filmstrip finish; VS uses a fox/panda cinema quiz stage. |
...
| `api/play-purchase.js` / `api/play-store.js` | Legacy ad-removal verification plus current six-product Play verification, installation-bound supply grants, post-delivery consumption and hashed-token ledgers. |
| `store-catalog.js` / `MONETIZATION.md` | Approved quantities, descriptions, India price targets and regional Play setup/release limits. |
| `assets/shop/` | Six transparent generated pack/ad-removal illustrations; exact prompts recorded. |
| `test-store-server.js` / `test-store-browser.js` / `AdBreakPolicyTest.java` | Verified supply delivery/replay/failure fixtures, localized shop and phone layout, result ad hooks and persistent cadence. |
...
### September 21 content cleanup and Play testing status

The owner approved cleaning adult/sexual-violence definitions before submitting a closed test. `scripts/answer_safety.json` excludes 81 additional answer words; 62 reviewed overrides replace unsuitable or misleading meanings. The standard pool is now 10,106 (Normal 4,046; Hard 2,226; Extreme 3,834), with the same frequency thresholds and unchanged 15,232 accepted guesses. `scripts/answer-safety-audit.json` records the exact changes. The dependency-free `apply_answer_safety.py` applies reviewed changes without reordering/retiering survivors; the full WordNet generator also honors the policy. Packaging and tests reject these known unsuitable definitions. This is targeted curation, not a formal rating or an exhaustive human review of every entry. Ordinary biology, equality, non-graphic crime/violence, gambling and alcohol/tobacco references remain and must be declared accurately.

Saved solo puzzles with retained answers receive current clues and preserve guesses. A removed saved answer starts a fresh puzzle at the same Adventure level; wallet, completed words and progression counters are not reset. The smaller pool changes seeded Adventure routes and the Daily selection from this update onward. Existing server rooms retain routes/progress until their 24-hour expiry; removed-word clues no longer return a stored unsuitable definition, but old answers can still appear on completion. Publish the updated backend and let legacy rooms expire before claiming all active multiplayer content uses the new pool. New room routes exclude the removed answers.

The previous content-cleanup build was 1.0.2 / code 3; its local debug APK and debug AAB include that client, corrected 88-second music loop, current Bollywood bank/prices, identities, coin generation 10 and neutral age prompt. The prompt reads “Choose your age group. We use it for privacy settings, and it stays on this device.” It has no “ads disabled under 13” message; age-based native privacy/eligibility behavior remains. Android Studio regenerated `android/app/release/app-release.aab` as upload-key-signed version 1.0.2 / code 3 on September 21. A byte-identical copy is `release/SixthSense-v1.0.2-closed-test.aab` (33,032,313 bytes; SHA-256 `9914768d0f42f899d870ecd1301d9c86658b5d4a0df0206f1006d11646cb022a`). Studio reused its saved key/passwords; no signing secrets were extracted or recreated, and `android/keystore.properties` remains absent. The bundle retains sample ads for testing; Play accepted its upload and it is saved in the Alpha draft.

Play Console browser setup: Alpha targets India and uses self-join Google Group `sixthsense-alphacode-testers@googlegroups.com`, at https://groups.google.com/g/sixthsense-alphacode-testers . Anyone on the web can join through the direct URL; conversation visibility is members-only and posting/member-list visibility is owners-only. No invitations/messages were sent. Draft `1.0.2 - Closed Alpha` now contains signed version 3 (1.0.2); version 1 was removed from the draft. Saved privacy URL, Ads=Yes, no government/financial/health features, Word game category and support contact; dashboard now shows 10/11 setup tasks complete. IARC terms were accepted with explicit owner approval. After cleanup, the revised questionnaire and summary were saved to Publishing overview, pending review; the preview showed IARC Generic 3+, ESRB Everyone 10+, PEGI 3, and higher ratings in some other regions. This is not Play approval. It declares textual non-graphic violence and rare medical/alcohol/tobacco references, plus digital purchases without loot boxes or cash rewards. Gambling themes are No under Google’s imagery/activity definition; native voice/chat/media exchange and location sharing are absent. Short/full listing descriptions were saved as a draft for version 1.0.2. Dedicated reviewer access, ages 13–15/16–17/18+, Advertising ID and Data safety declarations are saved. The content-cleanup draft still needs store listing completion and review/submission. The newer 1.0.3 monetization source additionally needs external store/ad setup, real license tests and a new signed bundle before replacing code 3. The user requested happy-fox icons above SIXTH SENSE blocks and no advertisements in screenshots; the three original concepts remain in `store-assets/fox-options/`. The owner has selected the furry happy-fox option, now applied locally with final exports in `store-assets/fox-selected/` and `store-assets/`. The previous aperture icon was uploaded to the asset library but not added to the listing. Icon selection is complete. Upload the new icon/feature graphic and ad-free screenshots before submission; the code-4 monetization build still needs signing and external activation/testing. No closed release was sent for review; dashboard showed zero closed testers, so the required 14-day period has not started. The future opt-in URL is https://play.google.com/apps/testing/com.sensei.sixthsense and is not yet an active installation path.


### Bollywood Race and VS exception

September 19 owner-approved replacement: **movies and fictional characters only**, one short, slightly vague Sense clue per word, no quiz questions or actor-name answers. The owner requested replacement of the entire earlier pool and a commit. `docs/bollywood-review/` preserves the approved words, clues and references in JSON, Markdown and searchable HTML; runtime tests require exact word/kind/clue agreement. Names such as FARHAN and VICKY identify fictional roles through their clues, not performers. This change is implemented locally but not pushed or deployed yet.

`data/bollywood-answers.json` now contains 125 unique 5–7-letter answers: 75 complete movie titles and 50 character given names/nicknames. There are 98 references from 2000–2025 and 27 pre-2000 classics. All 53 earlier reviewed movie clues are retained, with 22 movies and 50 characters added; the old actor-based selection pool is fully replaced. Movie titles omit only spaces/punctuation (New York, Ra.One); no title fragments or series are selected. Clues are newly authored, with reference works and selected source URLs recorded. The owner-approved list includes enduring cult classics as well as hits; not every movie has an official Hit verdict. Exact canonical spellings are required; no transliteration aliases are implemented. `BOLLYWOOD_CONTENT.md` records editorial and factual-review limits.

The durable room model uses `mode=race` or `mode=vs`, with additive `answer_theme` defaulting to `classic` for all existing rooms. Bollywood rooms use `answer_theme=bollywood`; answer selection remains random without repeats within the shared server route. Endless VS appends a fresh Bollywood answer on both solve and Last Chance forfeit; it recycles this pool only after all entries are used. Snapshots expose only the active word length and generic answer kind before purchased hints. The browser uses that length for columns, submission and Peek exhaustion; scoring uses the actual answer length and standard-mode validation stays six-letter. Older clients lacking variable-length capability are told to refresh/update when joining these rooms. Vercel, Pages and mobile publish only the allow-listed client bundle; all omit the data folder. Pages packaging explicitly sets the Vercel API origin with `--web --pages`; Vercel web builds retain same-origin API requests and native builds retain the explicit Vercel endpoint. The source repository is public, which does not expose a room's randomly chosen active route. A server-only legacy archive preserves purchased clue lookup for the short-lived rooms created before the owner’s refinement arrived; it is never selected for new routes.

### Standard vocabulary

- Answer pool: exactly 10,106 unique, clueable, answer-safe six-letter words.
...
- Normal (`easy`): 4,046 eligible answers at Zipf 2.75 or higher. Hard (`medium`): 2,226 remaining answers at Zipf 2.0–2.74. Extreme: 3,834 final rare, specialist, archaic, or unusual answers.
...
- Sense clues must not reveal their answer or use broken placeholder text. The September 5 clue-only refresh processed all 10,106 entries: 10,082 strings changed, including 1,310 definition-text changes and 8,772 grammatical-label-only changes. It preserves the old word/tier sequence, verified by SHA-256 in test-progression.js. The earlier August counts in the change log describe historical releases, not this refresh.
...
Statistics include Daily play/win/streak fields, the last rewarded seven-day milestone, persistent guess distribution (including historical seventh/eighth buckets), wallet, economy version, cumulative points, persistent lifeline inventory, Streak-mode run, best Streak-mode run, an Adventure seed/current level, the unique solved-answer list used for silent tier progression, total solves, best attempt count, and the fastest timed word. Economy version 10 assigns all older saved wallets exactly 500 coins once while preserving every other stored field, including the prior rewarded-claim ledger. Version-10 balances then earn/spend and persist normally. The reset applies separately to each site/device on updated-client load or through the public reset policy. Already-open policy-aware web pages check within 30 seconds while visible and on foreground/online recovery; clients predating that checker need refresh. Bundled Android installations do not download this web release; no Android update or Play upload is included in this web-only reset. New fields use backward-compatible defaults. Settings include hard mode, contrast, dark mode, independently stored music/effects, selected animal and color, selected avatar decoration, and locally unlocked premium avatars/decorations. The former single `sound` preference still migrates safely.
...
- Keep Normal threshold-based at Zipf 2.75 (currently 4,046 words), Hard at Zipf 2.0–2.74 (currently 2,226), and preserve all clueable answer-safe words in the complete tiered answer bank unless an explicit product decision changes the threshold.
...
September 21 selected-fox update: opaque Play icon is 512×512 RGB / 345,465 bytes; feature graphic is 1024×500 RGB / 504,422 bytes. All Android density foregrounds use actual alpha and fit the central safe region; selected square and round previews were inspected. `android:sync`, debug assembly, lint, icon-reference/export checks and whitespace passed. Rebuilt test APK is `release/SixthSense-v1.0.3-fox-debug.apk`, SHA-256 `29b7db58deb1caaa89057bacf2673413395d2f84d0cc36de63c764cc7e049950`. Earlier non-fox debug and signed code-3 artifacts are preserved; no new signed release or Play asset upload is claimed.

Release verification: feature commit `4574141` pushed to `main`; Vercel status success and GitHub Pages run `35634571602` success. Public `store-catalog.js`/`mobile.js` bytes on both sites match the committed source. `/api/play-store` returns HTTP 503 with `available:false` and catalog version 1 until credentials are configured. This deployment does not update the installed Android app or activate purchases.

September 21 monetization source 1.0.3/code 4: `npm test`, legacy `test:purchases`, new `test:store` server/browser fixtures, `test:mobile`, `test:last-chance`, `test-banner-layout.js`, `test-wallet-reset.js`, syntax and whitespace checks passed. Browser store checks cover all five supply receipts, localized-price changes, permanent-owned controls, duplicate/reload recovery, atomic storage failure, full-wallet deferral, paid-wallet migration/reset exclusion, 320/390 px shop layout, real Adventure completion hooks and full-match race result hooks with failed-ad continuation. Native `AdBreakPolicyTest` has two passing tests for three-level cadence, persistent count and event deduplication. `android:sync`, `:app:testDebugUnitTest`, `:app:assembleDebug` and `:app:lintDebug` passed. Generated shop image alpha and the rendered phone shop were inspected. APK web entries match the generated mobile bundle and include six shop illustrations without backend/signing files. The debug APK is `release/SixthSense-v1.0.3-debug.apk`, SHA-256 `2ab973f073cccfbc26d6c6dfe716f92f0365cf3e15c0e906ae40337098f479f5`. No signed code-4 upload, real Play payment, new native interstitial presentation or physical-device acceptance is claimed. Merchant profile and live configuration remain pending.

September 21 current Console verification: all Data safety pages saved, dashboard 10/11 complete. Name/User IDs, purchase history and multiplayer actions are optional collected data; approximate location, app interactions, diagnostics and device IDs are collected/shared for SDK ads, analytics and fraud prevention. HTTPS in transit and manual deletion-request URLs declared; public deletion page deployed on Vercel. No independent security badge claimed. Reviewer credentials remain Console-only. Native API 36 emulator completed ORANGE/PLAITS/CLAMPS Daily guesses, a 30-coin Sense purchase and victory; genuine clue/victory/map captures contain no ad creative. Advertising screenshots are excluded, per user. Three fox logo concepts are previews only; final store icon, native icon update and release submission await selection. No review submission or 14-day test start is claimed.

September 21 closed-test preparation (initial checkpoint, superseded by the completed declarations below): Play accepted signed version 3 (1.0.2), which replaced version 1 in the saved Alpha draft. Two dedicated reviewer profiles and recovery-code access instructions were saved privately in Console; credentials are not tracked. Ages 13–15/16–17/18+ and Advertising ID usage (analytics, advertising, fraud prevention) were saved. Data safety and store graphics remain in progress; no review submission. Added a public account/data deletion request page using the existing support-email process; web packaging includes it and the already-signed native bundle is unchanged. Web build and whitespace checks passed. Deployment verification follows separately.
...
September 19 unified gameplay messages (styles/app/multiplayer `20260919.1`): local focused browser checks passed actual invalid input in Daily, Race, VS, Co-op, Bollywood Race and Bollywood VS, identical bubble styles, feedback surviving changed snapshots, timer restoration, Clear copy, recovery from a failed poll and exit cleanup. Light/dark 320×568, 390×844 and 844×360 message bounds, no page overflow and stable board geometry passed; the dark Race screenshot was visually inspected. Core/identity, full browser, Peek, presence, native banner geometry, keyboard touch and Race mobile suites passed. The first concurrent Race touch run reported one missing A; an independent rerun passed all rapid/edge/poll-touch assertions without an input-code change, so physical-device acceptance remains unclaimed. The Peek fixture now waits for an actual revealed tile after the network response. JavaScript syntax, whitespace and native/Vercel/Pages packaging passed. No new native binary or live Google/ad integration is included. Source commit `163a0ef` deployed successfully to Vercel and Pages (workflow `35445144876`); both serve runtime `20260919.1`. The focused suite passed against each public build with explicit room/identity fixtures, not newly created production matches. Public verification exposed timing-only test issues during responsive fitting and waiting for full page load; the test now waits for observer-sized board bounds and uses DOM-ready navigation.

September 18 global usernames (`20260918.16` identity/app; `20260918.17` CSS/multiplayer after the compact-phone follow-up): unit checks passed for normalized collision keys, invalid/reserved names, hashed credentials, atomic claim/rename query shape, server-derived names, forged-owner denial and protected legacy-seat binding. Browser fixtures passed duplicate case/space claims, rejected-rename preservation, invalid/valid recovery, cross-device profile reuse, unchanged wallet, reload and 320px recovery layout. Core/data/VS, room resume, touch and banner checks passed. Full browser, Race mobile, presence, syntax and native/Vercel/Pages bundle checks also passed. The bundle includes identity.js and excludes server identity code/data. Preview build 547d8af passed, but Vercel sign-in protection returns HTTP 401 before reaching the API, so no preview database test is claimed. Production 5f296bb deployed successfully on Vercel and Pages (workflow 35368542940). Live tests passed concurrent global claims (one success/one conflict), case/space duplicates, failed-rename retention, claim retries, invalid-code denial, forged profile/name rejection, names across multiple rooms, cross-device linked-seat/progress recovery, idempotent replacement seat credentials, old-key invalidation and rename propagation. A QA seat created before deployment retained its id/attempts/progress through guest-label-to-profile migration. Actual two-browser Bollywood VS passed on each public origin with registered names, Sense/Peek, solving/scoring, next round and reload resume. The live Race suite also passed late join, duplicate global-name rejection, concurrent eight-seat capacity, preserved existing progress and full-room leave/reload/code rejoin. No native binary rebuilt.
...
- Bollywood's 125-word replacement has owner-reviewed hints, per-entry reference credits and selected source URLs, not an exhaustive independent factual audit. Character-name familiarity and Latin spellings vary; only the listed canonical spelling solves the word. Preserve complete movie titles or fictional character names/nicknames and 5–7 letters. Existing stored routes retain lookup through the old server-only archive until room expiry; that archive never supplies new answers. The replacement is local and not deployed yet.
...
- Android 1.0.3 automatic ads, six-product shop and backend delivery are implemented. Merchant profile, catalog activation/regional prices, service credentials, real interstitial ID and live SDK/license tests remain pending; `MONETIZATION.md` records exact setup. The signed Alpha draft is still older code 3, and no code-4 release has been uploaded. This is not a production-ready monetized Play release.
...
- Paid supply checkout is implemented with Google-verified, installation-bound receipt delivery but is not activated. Coins/lifelines remain device-local, not a server-authoritative or cloud-restorable wallet. Native backup excludes them; reinstall/data clearing can lose supplies. Permanent ad removal restores separately. RTDN and consumed-supply refund clawbacks are not implemented.
...
### 2026-09-21 — Apply the selected happy-fox identity

- Preserved the owner's exact selected attachment and applied it to the Play icon, browser favicon/touch icon and web-app install icons. Added a matching generated feature graphic and a transparent adaptive launcher foreground with complete ears and mask-safe spacing.
- Replaced Android launcher density assets and connected the splash to the new adaptive icon. Removed obsolete API-24 XML overrides so legacy devices use the appropriate density/round bitmap exports. Kept in-game wordmark/avatar layout intact.
- Recorded exact built-in image_gen prompts and added deterministic Pillow exports. Verified opaque Play dimensions/file limits, actual alpha, square/round previews, native sync/build/lint and whitespace. Built a separate fox debug APK; icon selection is resolved, while Play asset upload, signed code-4 release and monetization/account readiness remain outstanding.

### 2026-09-21 — Approved regional shop and automatic end-of-game ads

- Implemented the owner's approved six-product catalog with localized Play prices, illustrated shop access from the native wallet, complete-pack supply bonuses and permanent banner/interstitial removal. Optional rewarded bonuses remain.
- Added whole-match and every-third-new-Adventure interstitial opportunities, durable event deduplication, no-fill continuation, consent/foreground/ownership guards and production-ID validation. Bumped Android source to 1.0.3/code 4 and cache versions to 20260921.3.
- Added Google-verified installation-bound supply delivery, native recovery and atomic wallet/receipt writes before consumption; paid packs never truncate at the wallet cap and paid wallets skip test resets. Legacy purchase endpoint remains compatible.
- Generated six transparent shop assets and retained exact prompts. Updated privacy and release/setup documentation to disclose installation-local supplies, permanent restore and the missing merchant profile/service credentials/interstitial unit/license tests. The owner's payments setup is a handoff, not a completed financial account.
- Store server/browser, mobile reward, banner layout, Last Chance, wallet-reset and native cadence/compile/lint checks passed. Fixture checks do not establish real payment or interstitial delivery. Feature commit 4574141 deployed on Vercel and Pages; catalog/mobile bytes verified and checkout readiness remains unavailable. Existing code-3 Play draft and unselected fox-logo concepts remain untouched.

### 2026-09-21 — Save Data safety and explore the requested happy-fox icon

- Completed Data safety, reviewed the preview and verified dashboard 10/11 tasks complete. The upload-key-signed code 3 is accepted in the saved Alpha draft; no review submission.
- Created three happy-fox/SIXTH SENSE-block icon options with built-in image generation. These are unselected concepts; existing game/native artwork is unchanged. Store submission waits for the user's icon choice.
- Captured actual native clue, victory and map screens; excluded the test-banner gameplay capture from store assets. Store screenshots must contain no ad creative, as requested.
- Updated release handoff, image provenance and verification status; no gameplay code change or new release binary.
...
- Used Android Studio with the existing saved upload key to generate release 1.0.2/code 3, preserving the internal sample-ad configuration and production checklist gate.
- Verified signing certificate continuity, archive integrity and cleaned packaged assets; copied the signed result to `release/SixthSense-v1.0.2-closed-test.aab` for easy upload.
- Updated release handoff to distinguish the newly signed artifact from the still-old Play draft. Upload, remaining Console declarations/graphics and review submission are outstanding.
...
- Configured the self-join tester Group and India Alpha draft; saved six app-setup declarations. No closed release or production approval yet.
- Accepted IARC terms with owner confirmation. Inspection of the uploaded version 1 found adult definitions; the owner approved cleanup before submission. Removed 81 answer candidates and rewrote 62 clues; retained broad guesses and neutral biology/equality terms.
- Added reproducible policy/audit/build guards, refreshed retained saved clues, and prevented removed solo answers/old server clue text from returning. Legacy room answers may remain until expiry, and route/Daily selection changes are documented.
- Bumped Android to 1.0.2 / code 3, synchronized the latest client, built and checked debug APK/AAB and lint. Verified age-screen text does not advertise ad exemptions. A Play-uploadable signed release still needs the owner's private signing step.

### 2026-09-21 — Fresh 500-coin reset

- Advanced compiled economy/reset generation and public `economy.json` from 9 to 10 for the owner’s fresh reset request. Earlier-generation wallets become exactly 500 once, including balances above 500. Later earnings/spending and other saved progress remain intact.
- Versioned `app.js` as `20260921.1`, updated current-generation test fixtures and added v9 to migration coverage. Production delivery is checked with zero/high wallets left open on both sites before publication. No Android binary or Play update is included.
...
- Applied the requested 2.5x price multiplier in Bollywood Race and VS: Sense 75, Peek 125 and Clear 100 coins. Standard prices and the 125-coin Last Chance remain unchanged.
- Shared theme-aware pricing across the multiplayer dock, accessible names, wallet purchase and help copy; preserved stored-token use and free Sense reopening. Versioned game-core/app/multiplayer URLs as `20260919.2`.
- Added core and browser pricing regressions; verification and remaining delivery limitations are recorded above.

### 2026-09-19 — Replace Bollywood pool with movies and characters

- Replaced the complete 579-answer selection bank with the owner-approved 125 words: 75 whole movie titles and 50 fictional character names/nicknames, each with one short Sense clue. New Race, VS and Endless selections exclude the old actor answers.
- Updated home/help/lobby copy and contributor rules for movies/characters while preserving 5–7-letter input, six tries/Last Chance and the existing board/status rendering. Existing saved routes retain lookup through the already-present legacy archive; no old pool participates in new selection.
- Saved the reference JSON/Markdown/HTML outside client bundles and added exact-bank agreement, actor exclusion, character Sense/redaction, VS and responsive browser regressions. Core and focused checks passed locally; no push, deployment or native binary in this commit.

### 2026-09-19 — Consistent gameplay messages

- Replaced multiplayer's plain status text with the same bounded purple bubble used by solo, including shared invalid-guess and Peek/Clear wording. Kept message space fixed in portrait, landscape and native banner layouts so text does not push the board or keyboard.
- Centralized transient-message timing and room-status restoration. Room polling no longer overwrites temporary feedback; successful reconnection clears stale connection errors. Room/screen exits clear feedback, and multiplayer wallet feedback uses the visible online bubble.
- Verified successful Vercel/Pages publication and public-build message regressions; hardened browser waits for responsive board fitting and DOM-ready navigation.
- Added focused browser coverage and retained separate clue, presence, transition and result surfaces. Corrected an existing short-landscape solo overflow using shared fitted-board styling, and made the Peek browser fixture wait for its response-rendered tile instead of mistaking the temporary busy state for completion. No game rules, keyboard activation, coins, accounts, ad configuration or native binary are changed. Verification and deployment results are recorded above.

### 2026-09-18 — Verify global identity and compact phone onboarding

- Verified registry concurrency, recovery and room ownership with the real production database, migrated a pre-deployment QA room seat without resetting progress, and exercised registered multiplayer on both public sites. Recorded results in Verification.
- Kept reserve/restore/guest controls visible at 320×568 by reducing onboarding artwork and copy spacing. Removed the old client-only Player placeholder rejection so that a legitimately reserved username Player can join normally. Focused recovery/phone tests passed; native binaries still require an update.
...
- Removed Practice, Time Tackle, Insight and Streak from home/help at the owner’s request and replaced that shelf with Bollywood Race and Bollywood VS. Moved Race from Play Together to avoid a duplicate. Existing solo engines/saves remain compatible, with no wallet or progress reset.
- Generated and inspected two original animal/cinema illustrations using the built-in image tool; shipped optimized square WebP assets and retained exact prompts. Labels remain real accessible HTML. The shelf and card captions use deep purple surfaces with light text in dark mode; phone layout and both themes are supported.
- Added authoritative Bollywood VS with fixed or Endless rounds using the existing curated 579 answers. Both solve and forfeit advancement retain the theme; opponent feedback patterns accommodate seven letters. Existing room security, Last Chance, hints, presence, resume and click-only input remain.
- Updated home/browser expectations and retained solo regression coverage through a test-only fixture. Current verification and release limitations are recorded above.
