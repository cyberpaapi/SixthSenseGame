...
- Game screen: visible Sixth Sense branding, a compact game-mode line, a phone-safe five-digit coin count at top right, seven-row letter grid, a full-width color-state keyboard, and an evenly spaced bottom lifeline dock. The header now keeps only Help and Settings beside the wallet; Statistics is a full-width clay action inside Settings. The three keyboard rows contain 27 controls—26 letters plus Delete—with a proportionally centered final row; the removed Enter key is unnecessary because the sixth letter submits automatically.
...
- Purposeful motion includes letter entry pop, row rejection shake, tile reveal, Peek reveal, Clear key removal, combined lifeline purchase/use/unavailable states, wallet spend/denial, screen entry, staged result-word tiles, a full-screen solve burst, a delayed two-sided confetti cannon timed to the victory card, and a lighter in-sheet cascade. The completion flow also uses a short optional vibration pattern where supported.
...
- Vercel production: primary release target at `https://sixth-sense-game.vercel.app/`; runtime `20260831.6` contains the five-digit wallet cap and Statistics-in-Settings release plus the current Adventure and completion releases described above.
