...
- Multiplayer production: activated through the Vercel Neon integration on its free plan with `DATABASE_URL` connected to Production, Preview, and Development. The latest public-URL run passed create/join/start with two isolated clients, refresh rejoin, 2,234ms opponent-attempt visibility, synchronized VS advancement, live Sense state with no answer field, and premium chameleon/Champion identity synchronization after the schema migration.
